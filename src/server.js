...
function classForMsg(msg) {
  if (msg.includes("Complete")) return "ok";
  if (msg.includes("Fatal") || msg.includes("error")) return "err";
  if (msg.includes("Cancelled") || msg.includes("Warning")) return "warn";
  if (msg.includes("Progress") || msg.includes("Starting")) return "info";
  return "";
}
...
async function startBackfill() {
  ...
  clearLog();
  setRunning(true);
  appendLog("[UI] Starting backfill: " + startDate + " to " + endDate);
...
async function loadHistory() {
  ...
  tbody.innerHTML = data.runs.map(r => {
    ...
    const s = r.stats || {};
    return "<tr><td>" + (r.startDate || "?") + " to " + (r.endDate || "?") + "</td><td>" + started + "</td><td><span class='tag " + (r.status || "") + "'>" + (r.status || "-") + "</span></td><td>" + (s.total ?? "-") + "</td><td>" + (s.attached ?? "-") + "</td><td>" + (s.errors ?? "-") + "</td><td>" + (r.status === "cancelled" || r.status === "error" ? "<button class='btn btn-secondary resume-btn' onclick=\"resumeRun('" + r.runId + "','" + r.startDate + "','" + r.endDate + "')\">Resume</button>" : "") + "</td></tr>";
  }).join("");
}
