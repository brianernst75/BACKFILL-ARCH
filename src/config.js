...
export function isCustomerCall(from, to) {
  ...
  const fromIsAgent = AGENT_EXTENSIONS.has(fromNorm) || AGENT_DIDS.has(fromNorm) || RING_GROUPS.has(fromNorm);
  const toIsAgent   = AGENT_EXTENSIONS.has(toNorm) || AGENT_DIDS.has(toNorm) || RING_GROUPS.has(toNorm);
